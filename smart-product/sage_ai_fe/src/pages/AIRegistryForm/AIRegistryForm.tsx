...
import styles from './AIRegistryForm.module.scss'

const AIRegistryFormInner: React.FC = () => {
  ...
  // Retain initial form snapshot for diff/reset logic; currently used to initialize updatedFormData
  ...
  const [approvalIndex, setApprovalIndex] = useState<string | undefined>(
  ...
  const header: FormHeaderProps = {
    formTitle: 'AI Registry and Tech Innovation Pipeline ',
    formSubtitle: (
      <>
  ...
  const handleAIFormSubmit = async (e: Record<string, unknown>) => {
    try {
      if (!formSchema) {
      ...
      // Get coverage scores from context
      // TODO: need to implement coverage scores usage post actual implementation
      // const coverageScores = aiFeatures.getCoverageScores()
      ...
      const form_data = formQuestions
        .filter(item => NOVELTY_SCORE_QUESTION_ID.includes(item.questionId))
        .map(item => ({
          questionId: item.questionId,
          question: item.question,
          answer: item.answer[0],
        }))
      const noveltyScorePayload = {
        'form-data': form_data,
      ...
      const promises = [
        ...
        noveltyScoreApiService.getNoveltyScore(noveltyScorePayload),
      ]

      const results = await Promise.allSettled(promises)
      
      // Update approval index if score calculation was successful
      if (results[0].status === 'fulfilled') {
        const updatedScore = localStorage.getItem(
          `${APPROVAL_SCORE_PREFIX}-${FormDashboardFormType.AiRegistryForm}-${submissionId}`
        )
        setApprovalIndex(updatedScore || undefined)
      }
      ...
      showToast({
        addToast,
        message:
          'Your "AI Registry and Tech Innovation Pipeline" form has been submitted successfully.',
        variant: 'success',
      })

      // Navigate to dashboard on successful submission (use the existing submissionId)
      ...
      // Re-throw the error so the loading button shows failure state
      throw error
    }
  }
  ...
  const handleSaveDraft = async (e: Record<string, unknown>) => {
    try {
      ...
      // Calculate score and wait for completion before navigating
      try {
        ...
        // Update the approval index state after score calculation
        const updatedScore = localStorage.getItem(
          `${APPROVAL_SCORE_PREFIX}-${FormDashboardFormType.AiRegistryForm}-${submissionId}`
        )
        setApprovalIndex(updatedScore || undefined)
      } catch (scoreError) {
        console.error('Error calculating score during draft save:', scoreError)
        // Don't fail the draft save if scoring fails
      }
      
      // Navigate to dashboard AFTER score calculation is complete (or failed)
      navigate(`/form-dashboard/${submissionId}`)
    } catch (error) {
  ...
  // Button configuration logic per page - memoized with updatedFormData dependency
  ...
  // Parent-owned handlers for button actions (business logic lives here)
  const actionHandlerMap: Record<ButtonActionType, () => void | Promise<void>> =
    useMemo(
      () => ({
        ...
        [ButtonActionId.submit]: () => handleAIFormSubmit(updatedFormData),
        [ButtonActionId.skipUpload]: () => handleAIFormSubmit(updatedFormData),
  ...
  return (
    <main className={styles.mainContainer}>
      <FormContainer
        ...
        onFormSubmit={e => handleAIFormSubmit(e)}
