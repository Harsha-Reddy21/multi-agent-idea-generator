...
class ChatService:
    ...
    async def get_response(
        ...
        try:
            logger.info(f"[{request_id}] Generating response for message: {user_message[:200]}")

            query = user_message
            if document_content:
                query = f"{document_content}\n\nUser question: {user_message}"
            ...
            logger.info(f"[{request_id}] Final Query: {query}")
            logger.info(f"Length of query: {len(query)}")
            ...
            if not message and isinstance(response, dict):
                message = response.get("response", "") or response.get("text", "")
